...
{
    while(getNPCCount() < max_npcs_ && getRoomCount() < max_rooms_)
    {
        int random1 = randomNum();
        int random2 = randomNum();
        int random3 = randomNum();
        int random4 = randomNum();
        addRoom(random1, random2);
        addNPC(random3, random4);
    }
}
            ...
            { // NPC location, have to check if they were found yet
                for (int k = 0; k < npc_count_; k++)
                {
                    if (npc_positions_[k][0] == i && npc_positions_[k][1] == j)
                    {
                        ...
                        {
                            cout << "\033[32m" << UNEXPLORED << "\033[0m";
                        }
                    }
                }
            }
